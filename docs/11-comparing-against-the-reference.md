...
| A `[★ 0] [Recents 0]` row in the sidebar | Both chips are real links | `/games/recent` and `/games/favourite` both exist, and each badge is the length of the list its own page renders, so neither can disagree with what opening it shows. The reference keeps favourites server-side and refetches the count on a `favouriteToggle` window event; the platform has no favourites table, so `useFavourites` keeps the games in `localStorage` and the store's subscribers are the event |
